...
public:
	...
	void SetOriginPosition(const b2Vec2& position, Fixed rotation);
	...
	// This breaks any contacts and wakes the other bodies.
	void SetCenterPosition(const b2Vec2& position, Fixed rotation);
	...
	void SetAngularVelocity(Fixed w);
	Fixed GetAngularVelocity() const;

	// Apply a force at a world point. Additive.
	void ApplyForce(const b2Vec2& force, const b2Vec2& point);

	// Apply a torque. Additive.
	void ApplyTorque(Fixed torque);
	...
	Fixed m_mass, m_invMass;
	Fixed m_I, m_invI;

	Fixed m_linearDamping;
	Fixed m_angularDamping;
...
inline Fixed b2Body::GetRotation() const
{
	return m_rotation;
}
...
inline void b2Body::SetAngularVelocity(Fixed w)
{
	m_angularVelocity = w;
}

inline Fixed b2Body::GetAngularVelocity() const
{
...
inline void b2Body::ApplyTorque(Fixed torque)
{
	if (IsSleeping() == false)
	{
		m_torque += torque;
...
inline Fixed b2Body::GetMass() const
{
	return m_mass;
}

inline Fixed b2Body::GetInertia() const
{
	return m_I;
}
